...
async function execute(
	...
	if (!allowedChannels.includes(channel.id)) {
		...
		].Commands.Lead.VC.Rename.WrongChannel(channel, allowedChannels);
	}
	else {
		await channel.setName(
			...
			].Commands.Lead.VC.Rename.AuditLogRename(channel, interaction.user)
		);
		reply =
			Languages[interaction.language].Commands.Lead.VC.Rename.Success(
				channel
			);
	}
...
async function autocomplete(interaction: AutocompleteInteraction) {
	...
	if (stateChannel) {
		choices.push(
			`${stateChannel} ${
				Languages[interaction.guild.preferredLanguage].Objects.Meeting
			}`
		);
	}
	choices.push(
