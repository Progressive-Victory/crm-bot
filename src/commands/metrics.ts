...
async function execute(interaction: ChatInputCommandInteraction<'cached'>) {
	...
	if (user) {
		...
		reply.setTitle(i18n.t('user-embed-title', { lng: interaction.locale, ns }))
			.setThumbnail(userPFP)
			.setAuthor({ iconURL: userPFP, name: user.tag })
			.setFields(
				{
					name: i18n.t('user-embed-vc-name', { lng: interaction.locale, ns }),
					value: i18n.t('user-embed-vc-value', {
						lng: interaction.locale, ns, joins: `${metrics?.vcJoins?.length ?? 0}`, leaves: `${metrics?.vcLeaves?.length ?? 0}`
					})
				},
				{
					name: i18n.t('user-embed-messages-name', { lng: interaction.locale, ns }),
					value: i18n.t('user-embed-messages-value', {
						lng: interaction.locale, ns, messages: `${metrics?.messages?.count ?? 0}`
					})
				},
				{
					name: i18n.t('user-embed-server-name', { lng: interaction.locale, ns }),
					value: i18n.t('user-embed-server-value', {
						lng: interaction.locale, ns, joins: `${metrics?.joins?.length ?? 0}`, leaves: `${metrics?.leaves?.length ?? 0}`
					})
				},
				{
					name: i18n.t('user-embed-connected-name', { lng: interaction.locale, ns }),
					value: i18n.t('user-embed-connected-value', {
						lng: interaction.locale, ns, connected: `${await checkConnected(user.id, guild.id) ? 'Yes' : 'No'}`
					})
				}
			)
			.setTimestamp();
		...
		reply.setTitle(i18n.t('server-embed-title', { lng: interaction.locale, ns }))
			.setThumbnail(guild.iconURL({ forceStatic: true, size: 1024 }))
			.setFields(
				{
					name: i18n.t('server-embed-members-count-name', { lng: interaction.locale, ns }),
					value: i18n.t('server-embed-members-count-value', {
						lng: interaction.locale, ns, leaves: `${metrics?.leaves?.length ?? 0}`, membercount: `${guild.memberCount}`
					})
				},
				{
					name: i18n.t('server-embed-in-name', { lng: interaction.locale, ns }),
				...
				{
					name: i18n.t('server-embed-out-name', { lng: interaction.locale, ns }),
					value: i18n.t('server-embed-user-value', {
						lng: interaction.locale,
						ns,
						connected: `${usersInServerButConnected.length}`,
