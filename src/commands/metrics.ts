...
export default new Command({
	...
	execute: async function execute(
		...
		const commandLanguage =
			Languages[interaction.language].Commands.Metrics;

		let str = `${commandLanguage.Title()}\n `;

		if (user) {
			str += `${commandLanguage.User()}: **${user.tag}** (${user.id})\n`;
			str += `**${commandLanguage.VCJoins()}**: ${
				metrics?.vcJoins?.length ?? 0
			}\n`;
			str += `**${commandLanguage.VCLeaves()}**: ${
				metrics?.vcLeaves?.length ?? 0
			}\n`;
			// str += `**VC Time**: ${metrics?.vcTime ?? 0} seconds\n`; // TODO
			str += `**${commandLanguage.Messages()}**: ${
				metrics?.messages?.count ?? 0
			}\n`;
			str += `**${commandLanguage.ServerJoins()}**: ${
				metrics?.joins?.length ?? 0
			}\n`;
			str += `**${commandLanguage.ServerLeaves()}**: ${
				metrics?.leaves?.length ?? 0
			}\n`;
			str += `**${commandLanguage.ConnectedForm()}?**: ${
				(await checkConnected(user.id, guild.id)) ? 'Yes' : 'No'
			}\n`;
		}
		else {
			str += `${commandLanguage.Server()} **${guild.name}** (${
				guild.id
			})\n`;
			str += `**${commandLanguage.LeavesToMemberCount()}**: ${
				metrics?.leaves?.length ?? 0
			}/${guild.memberCount}\n`;
			...
			str += `**${commandLanguage.Connected()}**: ${
				usersInServerButConnected.length
			}\n`;
			str += `**${commandLanguage.NotConnected()}**: ${
				usersInServerButNotConnected.length
			}\n`;
			str += `**${commandLanguage.VCJoins()}**: ${
				metrics.vcJoins.filter((row) => memberIDs.includes(row.userID))
					.length
			}\n`;
			str += `**${commandLanguage.VCLeaves()}**: ${
				metrics.vcLeaves.filter((row) => memberIDs.includes(row.userID))
					.length
			}\n`;
			str += `**${commandLanguage.Messages()}?**: ${
				metrics.messages.filter((row) => memberIDs.includes(row.userID))
					.length
			}\n`;
			str += '\n';
			...
			str += `**${commandLanguage.Connected()}**: ${
				usersNotInServerButConnected.length
			}\n`;
			str += `**${commandLanguage.NotConnected()}**: ${
				usersNotInServerButNotConnected.length
			}\n`;
			str += `**${commandLanguage.VCJoins()}**: ${
				metrics.vcJoins.filter((row) => !memberIDs.includes(row.userID))
					.length
