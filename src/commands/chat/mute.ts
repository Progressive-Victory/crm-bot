...
export const mute = new ChatInputCommand({
	...
	execute: async (interaction)=>{
		...
		if (!(targetMember instanceof GuildMember)) {
			targetMember = await guild.members.fetch(interaction.options.getUser('user', true))
		}

		// check that target is not a admin or bot
		if (targetMember.user.bot || targetMember.permissions.has(PermissionsBitField.Flags.Administrator)) {
...
async function logMessage(targetMember:GuildMember, mutingMember:GuildMember, durationMinutes:number, reason:string ){
	...
	const settings = await GuildSetting.findOne({guildId: targetMember.guild.id})
	if (!settings?.logging.timeoutChannelId) return

	// check that channel is real
	const timeoutChannel = await getGuildChannel(targetMember.guild, settings.logging.timeoutChannelId)
...
function vcMessage(targetMember:GuildMember, mutingMember:GuildMember, durationMinutes:number){
	...
	const footer = new TextDisplayBuilder().setContent(subtext(`User ID: ${targetMember.id}`))
