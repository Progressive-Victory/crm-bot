import { ApplicationCommandType, ContextMenuCommandBuilder, InteractionContextType, MessageContextMenuCommandInteraction, MessageFlags, UserContextMenuCommandInteraction } from 'discord.js';
...
export const reportUser = new ContextMenuCommand<UserContextMenuCommandInteraction>({
	...
	execute: (interaction) => {
		...
		modal.setCustomId(AddSplitCustomId(reportModalPrefix.userReport, interaction.targetId))
...
export const reportMessage = new ContextMenuCommand<MessageContextMenuCommandInteraction>({
	...
	execute: (interaction) => {
		...
		const modal = reportModel
		modal.setCustomId(AddSplitCustomId(reportModalPrefix.messageReport, interaction.targetMessage.channelId, interaction.targetMessage.id))
