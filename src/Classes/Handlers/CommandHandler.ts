...
export class CommandHandler {
   ...
   */
  private validateAplicationCommand(
    AplicationCommand: ContextMenuCommand | ChatInputCommand,
    type: ApplicationCommandType
  ) {
    assert(
      typeof AplicationCommand.execute !== "undefined",
      "excute function not present"
    );
    assert(
      typeof AplicationCommand.builder !== "undefined",
      "builder is not present"
    );
    if (AplicationCommand.type !== type)
  ...
  addChatCommands(commands: Collection<string, ChatInputCommand>) {
    for (const [name, command] of commands)
      try {
        this.validateAplicationCommand(
          command,
