import { ChannelType, ChatInputCommandInteraction, quote } from "discord.js";
...
export default async function ping(
  ...
  const { locale, options, guild, member } = interaction;
  ...
  const role = guild.roles.cache.find(
    (r) => stateFromChannel.abbreviation === r.name.toLowerCase()
  );

  // check to see if the person trying to use the command has the role being pinged
  if (!member.roles.cache.has(role.id))
    return interaction.followUp({
      content: local.t("WrongRegionChannel", "common", {
  ...
  let temp: string;
  if (message) temp = quote(message) + "\n";
  else temp = "";

  const sent = await channel.send({
    content: `${role}\n${temp.toString()}${local.t("ping-sent-by", ns, {
