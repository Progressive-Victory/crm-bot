...
async function fetchAllMessages(channel: TextChannel | VoiceChannel) {
	...
	let message = channel.messages.cache.first();
...
export async function printChannel(channel: TextChannel | VoiceChannel) {
	...
	return new AttachmentBuilder(Buffer.from(messages.map((mes) => `${mes.createdAt.toUTCString()} ${mes.author.username}: ${mes.content}\n\n`).join()), { name: `${channel.name.replace(' ', '-')}-${channel.type === ChannelType.GuildText ? 'chat' : 'vc'}.txt` });
}
