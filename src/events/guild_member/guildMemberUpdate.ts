	...
	execute: async (oldMember, newMember) => {
		...
		if (oldMember.nickname !== newMember.nickname) {
			...
			if (nicknameLogChannel?.isSendable()) {
				...
				const description = `${newMember.toString()} ${newMember.user.username} changed their nickname from ${oldMember.nickname ?? oldMember.displayName} to ${newMember.nickname ?? newMember.displayName}`
				const avatarURL = newMember.displayAvatarURL({ forceStatic: true })
