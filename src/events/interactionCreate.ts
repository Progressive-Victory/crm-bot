	...
	try {
		...
		case InteractionType.ApplicationCommand:
			...
			case ApplicationCommandType.ChatInput:
				...
				interaction.client.commands
					...
					?.execute(interaction);
				Logger.debug(
					`Executed Slash Command ${interaction.commandName} by ${interaction.user.tag} (${interaction.user.id}) in ${interaction.guild?.name} (${interaction.guild?.id})`
				);
				break;
			...
			case ApplicationCommandType.User:
				interaction.client.contextMenus
					.get(interaction.commandName)
					?.execute(interaction);
				Logger.debug(
					`Executed Contex Menu Command ${interaction.commandName} by ${interaction.user.tag} (${interaction.user.id}) in ${interaction.guild?.name} (${interaction.guild?.id})`
				);
				break;
			default:
				break;
		...
		case InteractionType.MessageComponent:
			...
			case ComponentType.Button:
				...
				if (!interaction.client.receiveMessageComponents) {
					return;
				}
			...
			case ComponentType.StringSelect:
				interaction.client.selectMenus
					.get(interactionName)
					?.execute(interaction);
				break;
			default:
