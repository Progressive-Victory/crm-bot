import { ActionRowBuilder, bold, ButtonBuilder, ButtonInteraction, ContainerBuilder, ContainerComponent, EmbedBuilder, MessageFlags, TextDisplayBuilder } from "discord.js";
import { Interaction } from "../../Classes/index.js";
import { unWelcomeButton, welcomeButton, welcomeColors } from "../../features/welcome.js";
	...
	run: (interaction) => {
		...
		if (!message.flags.has(MessageFlags.IsComponentsV2)) {
			const embed = new EmbedBuilder(message.embeds[0].toJSON())
				embed.setColor(welcomeColors.Welcomed)
					.addFields(
			...
			const welcomer = new TextDisplayBuilder().setContent(`${bold('Welcomed By:')} ${interaction.member?.toString()}`)
			const row = new ActionRowBuilder<ButtonBuilder>().addComponents(unWelcomeButton)
			const container = new ContainerBuilder(component.toJSON())
				.spliceComponents(1, component.accentColor === welcomeColors.welcomeRemoved ? 1 : 0, welcomer)
...
export const unwelcome = new Interaction<ButtonInteraction>({
	...
	run: (interaction) => {
		...
		if (!(component instanceof ContainerComponent)) return
			...
			const welcomer = new TextDisplayBuilder().setContent(`${bold('Welcome Removed By:')} ${interaction.member?.toString()}`)
