...
export const usermove = new Interaction<UserSelectMenuInteraction>()
	...
	.setRun(async (interaction: UserSelectMenuInteraction) => {
		const {
			customId, guild, values, locale, user
		} = interaction;
		...
		const destination = guild.channels.cache.find(channel => channel.id === args[1]) as VoiceChannel;
		const source = guild.channels.cache.find(channel => channel.id === args[2]) as VoiceChannel;
		...
		await interaction.update({
			content: localize.t('reply_moved', ns, locale, { destination: destination.toString() } ),
			components: []
		});
	});
